...
public class MotionView : MonoBehaviour {
	...
	public bool TrailLinesConstantWidth = false;
	public bool TrailLinesFade = true;
	...
	void DrawPos_Trail(){
		...
		if(TrailLinesConstantWidth)
        {
			Pos_Trail_Renderer.startWidth = TrailLinesWidth;
		}
		else
        {
	...
	void DrawSpeed_Trail() {
		...
		if (TrailLinesConstantWidth)
		{
			Speed_Trail_Renderer.startWidth = TrailLinesWidth;
		}
		else
		{
	...
	void DrawRotation_Trail() {
		...
		for (int i = 0; i < Length-1; i++) { //Debug
